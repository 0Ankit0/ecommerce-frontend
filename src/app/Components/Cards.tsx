...
//   },
// ];
function Product_card({ title }: titleType) {
  const cards = useContext(ProductContext);
  const router = useRouter();
  return (
    <div className="container mx-auto my-10">
      <h1 className="my-2 text-2xl underline font-bold">{title}</h1>
      <div className=" grid lg:grid-cols-4 gap-6 md:grid-cols-3 sm:grid-cols-2">
        {cards.map((card, cardIndex) => (
          <div key={cardIndex} className="shadow-lg rounded-lg p-4 bg-white">
            ...
            <div className="mb-3">
              ...
              <div className="mt-5 flex gap-2 mb-3">
                <button className="px-4 py-3 tracking-wider  transition rounded-md bg-blue-500 hover:bg-blue-600">
                  Add to cart
                </button>
                ...
                <button
                  ...
                  onClick={() => router.push(`/ProductDetails/${card.id}`)}
                >
